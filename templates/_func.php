...
/**
 * Wrapper fn to size images in one place
 * Return resized image or do nothing if null image
 * @param string $str
 * @return string
 *
 * Author: P. Rushmer
...
/**
 * Truncate strings over a set length to indicate more text is available
 * Return truncated string + link or null string if not truncated
 * @param string $str
 * @return string
 *
 * Author: P. Rushmer
 *
 */
function strTrim($str, $cnt, $l, $t) {
	$txt = '';
	
	if (strlen($str) > $cnt)
	{
		$txt = substr($str, 0, $cnt);
		$txt .= "<a href= '$l'>$t</a>";
		return $txt;
	}
	else
